...
import { createBatch, CreateBatchPayload } from '../api/batches';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: CreateBatchPayload = {
        ...
        semester: formData.semester ? parseInt(formData.semester.toString()) : undefined,
      ...
      // Navigate to batch page after a short delay
      setTimeout(() => {
        navigate('batch-page');
      }, 1000);
    } catch (err: any) {
      const errorMsg = err.response?.data?.message || 'Failed to create batch. Please try again.';
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="bg-white/95 backdrop-blur-sm shadow-lg px-6 py-4 flex items-center justify-between">
        <h2 className="text-gray-800">Create New Batch</h2>
